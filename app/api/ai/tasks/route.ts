    ...
    if (userId) {
      try {
        await sql`
          ...
          VALUES (${userId}, 'ai_request', 'AI task planner request', ${JSON.stringify({ userProfile, projectContext })}, ${process.env.NVIDIA_MODEL || 'nvidia/nemotron-3-ultra-550b-a55b'})
        `
      } catch (e) {
    ...
    if (!result.success || result.usedFallback) {
      if (userId) {
        try {
          await sql`
            INSERT INTO ai_activity (user_id, type, description, input_data, model)
            VALUES (${userId}, ${result.usedFallback ? 'ai_fallback' : 'ai_error'}, ${result.error || 'AI fallback used'}, ${JSON.stringify({ userProfile, projectContext })}, ${process.env.NVIDIA_MODEL || 'nvidia/nemotron-3-ultra-550b-a55b'})
          `
        } catch (e) {
